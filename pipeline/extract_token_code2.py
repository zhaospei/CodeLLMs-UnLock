...
import dataeval.w_sec as dev_sec
...
from dataeval.w_sec import extract_generation_code as security_egc
...
def process_lfclf():
    ...
    dataset = get_dataset_fn(args.dataset)(
        tokenizer, language=args.language, instruction=instruction
    )
    ...
    for example in tqdm.tqdm(dataset, total=len(dataset)):
        ...
        task_generation_seqs_path = os.path.join(
            args.generate_dir, task_generation_seqs_path
        )
        ...
        for generated_ids in task_generation_seqs["generations_ids"]:
            ...
            clean_decode.append(clean_generation_decoded)
            start_ind, end_ind = getCleanGenerationRange(
                generated_ids.tolist(), clean_generation_decoded, tokenizer
            )
            if start_ind is None or end_ind is None:
                ...
                start, end = getGenerationRange(generated_ids.tolist(), tokenizer)
                clean_generations_range.append((start, end, has_error))
            else:
                clean_generations_range.append((start_ind, end_ind, has_error))

        clean_generations_range_all[task_id_path] = [
            clean_generations_range,
            clean_decode,
        ]
    for layer in args.layers:
        ...
        for example in tqdm.tqdm(dataset, total=len(dataset)):
            ...
            if args.dataset == "mbpp" or args.dataset == "ds1000":
                task_id_path = f"tensor({task_id_path})"
            task_generation_seqs_path = (
                f"generation_sequences_output_{task_id_path}.pkl"
            )
            task_generation_seqs_path = os.path.join(
                args.generate_dir, task_generation_seqs_path
            )
            ...
            with open(task_generation_seqs_path, "rb") as f:
                task_generation_seqs = pickle.load(f)
            found_sample += 1
            clean_generations_range = clean_generations_range_all[task_id_path][0]
            clean_generations_decode = clean_generations_range_all[task_id_path][1]
            ...
            for j in range(len(task_generation_seqs["generations_ids"])):
                ...
                if j >= len(task_generation_seqs["generations_ids"]):
                    print(len(task_generation_seqs["generations_ids"]))
                    print(len(task_generation_seqs["generations"]))
                    print(j)
                    print(task_generation_seqs_path)
                ...
                clean_decode = clean_generations_decode[j]
                start_code_ind, end_code_ind, has_error = clean_generations_range[j]
                start_ind, end_ind = getGenerationRange(
                    generated_ids.tolist(), tokenizer
                )
                num_tokens = end_ind - start_ind
                layer_embedding = task_embedding["layer_embeddings"][j]
                extracted_code = tokenizer.decode(
                    generated_ids.tolist()[start_code_ind:end_code_ind],
        ...
        results.to_parquet(
            os.path.join(
                output_dir,
                f"LFCLF_embedding_{args.dataset}_{model_name}_{layer}.parquet",
            )
        )
...
def process_last_line():
    ...
    PY_LANGUAGE = Language(
        "/home/trang-n/WCODELLM_MULTILANGUAGE/build/my-languages.so", "python"
    )
    ...
    for example in tqdm.tqdm(dataset, total=len(dataset)):
        ...
        if args.dataset == "dev_eval":
            ...
            function_name = get_function_name(
                example["original_prompt"].strip(), args.language
            )
        else:
        ...
        for generated_ids in task_generation_seqs["generations_ids"]:
            ...
            if function_name is None:
                function_name = get_function_name(
                    clean_generation_decoded, args.language
                )
            last_line_token_ids = getLineGenerationTokens(
    ...
    for layer in args.layers:
        ...
        for example in tqdm.tqdm(dataset, total=len(dataset)):
            ...
            for j in range(len(task_generation_seqs["generations"])):
                ...
                for id in last_line_token_ids:
                    ...
                    last_line_token_embeddings.append(
                        layer_embedding[chosen_id].tolist()
                    )
                results = results._append(
                    {
                        "task_id": task_id,
        ...
        results.to_parquet(
            os.path.join(
                output_dir,
                f"last_2_token_line_embedding_{args.dataset}_{model_name}_{layer}.parquet",
            )
        )
...
def process_min_prob_token():
    ...
    for example in tqdm.tqdm(dataset, total=len(dataset)):
        ...
        clean_generations_range = []
        for generated_ids, prob_scores in zip(
            task_generation_seqs["generations_ids"],
            task_generation_seqs["softmax_scores"],
        ):
            gen = tokenizer.decode(generated_ids, skip_special_tokens=True)
            clean_generation_decoded = dataset_egc(example, gen, args.language)
            start_ind, end_ind = getCleanGenerationRange(
            ...
            clean_generations_range.append(
                (start_ind, end_ind, int(min_prob_ind.item()), prob_score.item())
            )
    ...
    for layer in args.layers:
        ...
        for example in tqdm.tqdm(dataset, total=len(dataset)):
            ...
            for j in range(len(task_generation_seqs["generations"])):
                ...
                generated_ids = task_generation_seqs["generations_ids"][j]
                seq_prob_scores = (
                    torch.tensor(task_generation_seqs["softmax_scores"][j])
                    .numpy()
                    .tolist()
                )
                start_code_ind, end_code_ind, min_prob_ind, min_prob_score = (
                    clean_generations_range[j]
                )
                layer_embedding = task_embedding["layer_embeddings"][j]
                min_prob_ind = start_code_ind + min_prob_ind
                min_prob_line_token_embeddings = layer_embedding[
                    max(0, min_prob_ind - 1)
                ].tolist()
        ...
        results.to_parquet(
            os.path.join(
                output_dir,
                f"min_prob_token_embedding_{args.dataset}_{model_name}_{layer}.parquet",
            )
        )
...
def process_min_prob_token_line():
    ...
    for example in tqdm.tqdm(dataset, total=len(dataset)):
        ...
        for generated_ids, prob_scores in zip(
            ...
            for i in range(len(range_line_list) - 1):
                clean_generations_prob_scores = torch.tensor(
                    prob_scores[range_line_list[i] : range_line_list[i + 1] + 1]
                )
                prob_score, min_prob_ind = torch.min(
                    clean_generations_prob_scores, dim=0
                )
                min_prob_line_list.append(
                    (int(range_line_list[i] + min_prob_ind.item()), prob_score.item())
                )
    ...
    for layer in args.layers:
        ...
        for example in tqdm.tqdm(dataset, total=len(dataset)):
            ...
            for j in range(len(task_generation_seqs["generations"])):
                ...
                for min_prob_ind, min_prob_score in min_prob_line_list:
                    min_prob_line_token_embeddings.append(
                        layer_embedding[max(0, min_prob_ind - 1)].tolist()
                    )
        ...
        results.to_parquet(
            os.path.join(
                output_dir,
                f"min_prob_token_line_embedding_{args.dataset}_{model_name}_{layer}.parquet",
            )
        )
