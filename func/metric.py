...
rougeEvaluator = rouge_scorer.RougeScorer(
...
def getCleanGenerationRange(tokenized_generated_text, clean_text, tokenizer):
    try:
        tokenized_generated_text = tokenized_generated_text[
            : tokenized_generated_text.index(tokenizer.eos_token_id) + 1
        ]
    except:
        pass
    ...
    for i in range(
        ...
        - len(tokenized_clean_text)
        + min(len(tokenized_clean_text) // 2, 50)
    ):
        lo = i + 1
        ...
        while lo <= hi:
            mid = (lo + hi) // 2
            sub_generated_text = tokenizer.decode(
                tokenized_generated_text[i:mid], skip_special_tokens=True
            )
...
def getBodyRange(
    tokenized_generated_text, clean_text, tokenizer, parser, function_name
):
    if not f"def {function_name}(" in clean_text:
        ...
        for line in completion_lines:
            if count_indent(line) >= body_indent or line.strip() == "":
                ...
                break
        new_complation = "\n".join(new_complation)
        return getCleanGenerationRange(
            tokenized_generated_text, new_complation, tokenizer
        )
    else:
        func_body_clean_text = find_function_body(clean_text, function_name, parser)
        if func_body_clean_text is None:
            return getCleanGenerationRange(
                tokenized_generated_text, clean_text, tokenizer
            )
        return getCleanGenerationRange(
            tokenized_generated_text, func_body_clean_text, tokenizer
        )


def getLineGenerationTokens(
    tokenized_generated_text, clean_text, tokenizer, parser, function_name
):
    if function_name is None:
        ...
        start_ind, end_ind = getBodyRange(
            tokenized_generated_text, clean_text, tokenizer, parser, function_name
        )
    ...
    for i in range(start_ind, end_ind):
        if "\n" in tokenizer.decode(tokenized_generated_text[i : i + 1]):
            decoded_line = tokenizer.decode(
                tokenized_generated_text[fl_token:i]
            ).strip()
    ...
    for i in range(len(generated_texts)):
        for j in range(len(generated_texts)):
            if j <= i:
                continue
            LexicalSim += getRouge(
                rougeEvaluator, generated_texts[i], generated_texts[j]
            )
...
def getEigenIndicator(hidden_states):  # [num_tokens, 41, num_seq, [n/1], 5120]
    ...
    last_embeddings = torch.zeros(
        hidden_states[1][-1].shape[0], hidden_states[0][-1].shape[2]
    ).to("cuda")
    ...
    eigenIndicator = np.log10(
        np.linalg.det(CovMatrix + alpha * np.eye(CovMatrix.shape[0]))
    )
...
def getEigenIndicator_v0(hidden_states, num_tokens):
    ...
    for ind in range(hidden_states[1][-1].shape[0]):
        last_embeddings[ind, :] = hidden_states[num_tokens[ind] - 2][selected_layer][
            ind, 0, :
        ]
...
def getMiddleLayerEmbeddingEachToken(hidden_states, num_tokens):
    ...
    for ind in range(hidden_states[1][-1].shape[0]):
        ...
        for ind1 in range(num_tokens[ind]):
            ...
            last_embeddings[ind].append(
                hidden_states[ind1][selected_layer][ind, -1, :]
                .detach()
                .cpu()
                .float()
                .numpy()
    ...
    for ind in range(hidden_states[1][-1].shape[0]):
        for ind1 in range(len(hidden_states) - 1):
            if ind1 > num_tokens[ind] - 1:
                continue
            last_embeddings[ind, :] += hidden_states[ind1 + 1][selected_layer][
                ind, 0, :
            ]
        ...
        for seq_ind in range(hidden_states[1][-1].shape[0]):
            for token_ind in range(len(hidden_states) - 1):
                if token_ind > num_tokens[seq_ind] - 1:
                    continue
                last_embeddings[seq_ind, :] += hidden_states[token_ind + 1][layer_ind][
                    seq_ind, 0, :
                ]
            last_embeddings[seq_ind, :] = last_embeddings[seq_ind, :] / (
                num_tokens[seq_ind] - 1
            )
        CovMatrix = torch.cov(last_embeddings).cpu().numpy().astype(np.float)
        u, s, vT = np.linalg.svd(CovMatrix + alpha * np.eye(CovMatrix.shape[0]))
        eigenIndicator = np.mean(np.log10(s))
...
def getNumToken(generated_text, cleaned_text, tokenizer):
    tokenized_generated_text = tokenizer.tokenize(
...
def getEigenIndicator_v4(hidden_states):  # [num_tokens, 41, num_seq, ?, 5120]
    ...
    CovMatrix = (
        torch.cov(last_embeddings.transpose(0, 1)).cpu().numpy().astype(np.float)
    )
    u, s, vT = np.linalg.svd(CovMatrix + alpha * np.eye(CovMatrix.shape[0]))
    eigenIndicator = np.mean(np.log10(s))
    return eigenIndicator, s
...
def getEigenIndicator_v5(hidden_states, features):  # [num_tokens, 41, num_seq, ?, 5120]
    ...
    CovMatrix = CovMatrix + alpha * np.eye(CovMatrix.shape[0])
    pro = np.matmul(
        np.matmul((features - Mean).reshape(1, -1), np.linalg.inv(CovMatrix)),
...
def sample_selected(last_embeddings, features):
    ...
    for k in range(last_embeddings.shape[0]):
        dist.append(
            np.linalg.norm(last_embeddings[k, :] - features)
            + 1e-12 * np.random.random()
        )
...
def ParameterClip(model):
    ...
    for i in range(weight_importance.shape[0]):
        ...
        value_min, ind_min = torch.topk(
            torch.tensor(weight_importance[i, :]), k_low, largest=False
        )
        # weight_importance[i,:][weight_importance[i,:]>= value_max.numpy()[-1]] = 0
        weight_importance[i, :][
            weight_importance[i, :] <= value_min.numpy()[-1]
        ] = -1000
...
def ParameterClip_v1(model):
    ...
    k_low = int(
        ratio_low / 100 * weight_importance.shape[0] * weight_importance.shape[1]
    )
    # value_max, ind_max = torch.topk(torch.tensor(weight_importance.flatten()), k_high)
    value_min, ind_min = torch.topk(
        torch.tensor(weight_importance.flatten()), k_low, largest=False
    )
...
def ParameterClip_v2(model):
    ...
    value_min, ind_min = torch.topk(
        torch.tensor(weight_importance), k_low, largest=False
    )
    print(ind_max)
